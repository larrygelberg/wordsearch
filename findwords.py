...
def draw_capsule(ax, start, end, cols, rows, h_spacing, v_spacing, font_size):
    ...
    capsule_height = font_size * 0.055 * max(h_spacing, v_spacing)
...
def display_word_search(grid_file, words_file, name):
    ...
    def redraw():
        ...
        # Set up plot with adjusted limits
        ax.set_xlim(-0.5 * h_spacing[0], (cols - 0.5) * h_spacing[0])
        ax.set_ylim(-0.5 * v_spacing[0], (rows - 0.5) * v_spacing[0])
        ax.set_aspect('equal')
        ax.axis('off')
    ...
    # Create sliders
    ax_h_slider = plt.axes([0.15, 0.24, 0.7, 0.03])
    h_slider = Slider(ax_h_slider, 'H-Spacing', 0.5, 2.0, valinit=1.0, valstep=0.05)
    
    ax_v_slider = plt.axes([0.15, 0.19, 0.7, 0.03])
    v_slider = Slider(ax_v_slider, 'V-Spacing', 0.5, 2.0, valinit=1.0, valstep=0.05)
    
    ax_size_slider = plt.axes([0.15, 0.14, 0.7, 0.03])
    size_slider = Slider(ax_size_slider, 'Letter Size', 10, 40, valinit=20, valstep=1)
